<template>
    <div flex="~ row justify-center items-center" absolute h-full w-full top-0 hide-scrollbar>
        ...
        <button fixed z-9 top-45px left-10px w-40px h-40px dark:color-white dark:border-white dark:bg-black
            bg-transparent border-1px border-solid border-black color-black border-rd-6px hover:bg-black
            hover:color-white hover:cursor-pointer transition-all-250 v-show="showToggleButton"
            @click="showFormLayout = !showFormLayout">
            <div i-grommet-icons-menu relative w-full h-full />
        </button>
        <!-- 操作菜单切换显示按钮 -->
        <button z-9 fixed top-12 right-4 w-40px h-40px color-black dark:color-white dark:border-white dark:bg-black
            hover:bg-black hover:color-white hover:cursor-pointer border="black rd-2 solid 1px" transition-all-250
            @click="showActionMenu = !showActionMenu">
            <div i-grommet-icons-apps-rounded relative w-full h-full />
        </button>
        <FormLayout v-show="showFormLayout" m-r-5px z-8 p-t-10px p-b-25px position-relatives top-0
            overflow-y-auto w-400px flex="~ col justify-center items-center" w-fit
            class="form-layout"
            :class="`${isDark ? 'bg-rgba-63-63-63-0.4 color-white' : 'bg-rgba-0-0-0-0.6 color-white'}`">
            <div class="h-100%">
        ...
        <CardLayout card-layout flex="~ col justify-center items-center" h-full w-550px overflow-hidden>
            ...
            <div fixed h-fit p-3 right-15 top-15 bg-rgba-0-0-0-0.6 z-2 color-white v-if="showActionMenu">
                <p align="center" :class="`m-5px color-yellow ${locale === 'en' && 'font-size-12px'}`">{{ $t("cardHint")
                    }}</p>
                <p class="flex justify-center items-center hover:cursor-pointer m-5px">
                    <span>{{ $t("lockAspectRatio") }}: </span>
                    <input type="checkbox" v-model="store.lockAspectRatio" class="w-20px h-20px" />
                </p>
                <div flex="~ justify-center" mb-2>
                    <span>{{ $t("fileName") }}:</span>
                    <input type="text" v-model="store.exportFileName" m-inline-1 border="1px rd-1" />
                </div>
                <input type="file" @mouseenter="() => mask = `cursor-pointer upload-input-mask`"
                    @mouseleave="() => mask = ''" @change="handleUpload"
