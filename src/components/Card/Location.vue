<template>
    ...
    <template v-else>
        <!-- 左边 -->
        <img
            :src="dualClassLocationLeftURL"
            class="position-absolute top--20px z-1 pointer-events-none"
        />

        <!-- 分割线 -->
        <img
            src="@/assets/materials/location/location-dual-classes-split.png"
            class="position-absolute top-372px z-2 pointer-events-none"
        />

        <!-- 右边 -->
        <img
            :src="dualClassLocationRightURL"
            class="position-absolute top--20px z-1 translate-x--0.5px pointer-events-none"
        />

        <!-- 原画框 -->
        <img
            src="@/assets/materials/location/location-picture-frame.png"
    ...
    <template v-if="store.name">
        <p class="position-absolute pointer-events-none">
            ...
            <ArcText
                ...
                class="font-family-GBJenLei color-white hearth-stroke-1.2px top-312px z-4 font-size-29px position-absolute translate-x--200px translate-y--45px rotate--5deg"
            >
                {{ store.name }}
            </ArcText>
        </p>
    ...
    <template v-if="store.rarity !== Rarity.None">
        <!-- 稀有度遮罩 -->
        <img
            src="@/assets/materials/location/location-rarity-mask.png"
        ...
        <!-- 橙卡龙框 -->
        <img
            src="@/assets/materials/location/location-dragon.png"
    ...
    <!-- 法力值消耗数值 -->
    <div
        :class="`position-absolute top--22px z-3 font-size-57px pointer-events-none
    ...
    <!-- 地标名称框 -->
    <img
        src="@/assets/materials/location/location-name.png"
    ...
    <img
        ...
        src="@/assets/materials/location/location-description-area.png"
    ...
    <template v-if="store.description">
        <div
            class="position-absolute top-442px w-273px h-130px z-5 flex justify-center items-center"
        >
            <h3
    ...
    <!-- 数值 -->
    <div
        :class="`position-absolute top-458px z-4 font-size-48px
    ...
    <div v-if="store.cardFaceUrl" class="card-face-clip">
        <CardFace :url="store.cardFaceUrl" :card-type="CardType.Location" />
    </div>
    <!-- 遮罩 -->
    <Mask
        position-absolute
...
const locationBaseURL = computed<string>(
    () =>
        new URL(
            `../../assets/locations/single-class/${store.cardClass}-location.png`,
            import.meta.url,
        ).href,
);
...
const dualClassLocationLeftURL = computed<string>(
    () =>
        new URL(
            `../../assets/locations/dual-class/left/${store.dualCardClass.left}-location-left.png`,
            import.meta.url,
        ).href,
);
// 右卡底
const dualClassLocationRightURL = computed<string>(
    () =>
        new URL(
            `../../assets/locations/dual-class/right/${store.dualCardClass.right}-location-right.png`,
            import.meta.url,
        ).href,
);
