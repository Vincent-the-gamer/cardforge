<template>
    ...
    <img
        ...
        v-if="
            store.rarity === Rarity.Legandary ||
            (store.rarity === Rarity.None && store.showDragon)
        "
    />
    ...
    <Mask
        ...
        :card-type="CardType.BattlegroundMinion"
        v-show="
            store.cardType === CardType.BattlegroundMinion && store.showMask
        "
    />
</template>
...
const level = computed<string>(
    () =>
        new URL(
            `../../assets/materials/battlegrounds/level-${store.battlegroundLevel}.png`,
            import.meta.url,
        ).href,
);
