<template>
    ...
    <template v-if="store.name">
        <p class="position-absolute pointer-events-none">
            <span
                class="font-family-GBJenLei color-white hearth-stroke-1.2px top-314px z-4 font-size-30px position-absolute translate-x--200px w-400px h-40px flex justify-center items-center"
            >
    ...
    >
        <img
            src="@/assets/materials/weapon/weapon-kind.png"
            class="position-absolute top-556px z-4 pointer-events-none translate-x--1px"
        />
        <!-- 派系文字 -->
        <p
            class="position-absolute top-542px color-white translate-x-1px z-4 font-size-22px font-family-GBJenLei hearth-stroke-1.2px pointer-events-none"
        >
            {{ store.cardKind }}
        </p>
    </template>
    ...
    <template v-if="store.rarity !== Rarity.None">
        <!-- 稀有度遮罩 -->
        <img
            src="@/assets/materials/weapon/weapon-rarity-mask.png"
        ...
        <!-- 橙卡龙框 -->
        <img
            src="@/assets/materials/weapon/weapon-dragon.png"
    ...
    <!-- 随从名称框 -->
    <img
        src="@/assets/materials/weapon/weapon-name.png"
    ...
    <img
        ...
        src="@/assets/materials/weapon/weapon-description-area.png"
    ...
    <template v-if="store.description">
        <div
            class="position-absolute top-430px w-278px h-135px z-5 flex justify-center items-center"
        >
            <h3
    ...
    <!-- 攻击力，耐久底框 -->
    <img
        src="@/assets/materials/weapon/weapon-attack.png"
        class="position-absolute top-530px translate-x--160px z-4 pointer-events-none"
    />

    <img
        src="@/assets/materials/weapon/weapon-durability.png"
    ...
    <div v-if="store.cardFaceUrl" class="card-face-clip">
        <CardFace :url="store.cardFaceUrl" :card-type="CardType.Weapon" />
    </div>
    <!-- 遮罩 -->
    <Mask
        absolute
...
const weaponBaseURL = computed<string>(
    () =>
        new URL(
            `../../assets/weapons/single-class/${store.cardClass}-weapon.png`,
            import.meta.url,
        ).href,
);
...
const dualClassMinionLeftURL = computed<string>(
    () =>
        new URL(
            `../../assets/weapons/dual-class/left/${store.dualCardClass.left}-weapon-left.png`,
            import.meta.url,
        ).href,
);
// 右卡底
const dualClassMinionRightURL = computed<string>(
    () =>
        new URL(
            `../../assets/weapons/dual-class/right/${store.dualCardClass.right}-weapon-right.png`,
            import.meta.url,
        ).href,
);
