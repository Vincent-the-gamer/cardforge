<template>
    ...
    <template v-if="store.name">
        <p class="position-absolute pointer-events-none">
            ...
            <ArcText
                ...
                class="font-family-GBJenLei color-white hearth-stroke-1.2px top-312px z-4 font-size-28px position-absolute translate-x--240px translate-y--40px rotate--5deg"
            >
                {{ store.name }}
            </ArcText>
        </p>
    ...
    <template v-if="store.rarity !== Rarity.None">
        <!-- 稀有度遮罩 -->
        <img
            src="@/assets/materials/hero/hero-rarity-mask.png"
        ...
        <!-- 橙卡龙框 -->
        <img
            src="@/assets/materials/hero/hero-dragon.png"
    ...
    <!-- 随从名称框 -->
    <img
        src="@/assets/materials/hero/hero-name.png"
    ...
    <img
        ...
        src="@/assets/materials/hero/hero-description-area.png"
    ...
    <template v-if="store.description">
        <div
            class="position-absolute top-420px w-280px h-148px z-5 flex justify-center items-center"
        >
            <h3
    ...
    <img
        src="@/assets/materials/common/armor.png"
    ...
    <div
        :class="`position-absolute top-457px z-4 font-size-48px
    ...
    <div v-if="store.cardFaceUrl" class="card-face-clip">
        <CardFace :url="store.cardFaceUrl" :card-type="CardType.Hero" />
    </div>
    <!-- 遮罩 -->
    <Mask
        absolute
...
const heroBaseURL = computed<string>(
    () =>
        new URL(
            `../../assets/heroes/single-class/${store.cardClass}-hero.png`,
            import.meta.url,
        ).href,
);
...
const dualClassHeroLeftURL = computed<string>(
    () =>
        new URL(
            `../../assets/heroes/dual-class/left/${store.dualCardClass.left}-hero-left.png`,
            import.meta.url,
        ).href,
);
// 右卡底
const dualClassHeroRightURL = computed<string>(
    () =>
        new URL(
            `../../assets/heroes/dual-class/right/${store.dualCardClass.right}-hero-right.png`,
            import.meta.url,
        ).href,
);
