<template>
    ...
    <template v-else>
        <!-- 左边 -->
        <img
            :src="dualClassMinionLeftURL"
            class="position-absolute top--20px z-1 pointer-events-none"
        />

        <!-- 分割线 -->
        <img
            src="@/assets/materials/minion/dual-classes-split.png"
            class="position-absolute translate-x-2px top-395px z-2 pointer-events-none"
        />

        <!-- 右边 -->
        <img
            :src="dualClassMinionRightURL"
    ...
    <template v-if="store.name">
        <p class="position-absolute pointer-events-none">
            ...
            <ArcText
                ...
                class="font-family-GBJenLei color-white hearth-stroke-1.2px top-312px z-4 font-size-28px position-absolute translate-x--234px translate-y--40px rotate--5deg"
            >
                {{ store.name }}
            </ArcText>
        </p>
    ...
    <!-- 单种族 -->
    <template
        v-if="store.kindType === KindType.Single && store.cardKind.length > 0"
    >
        <img
            src="@/assets/materials/minion/minion-kind.png"
            class="position-absolute top-546px translate-x-7px z-4 pointer-events-none"
        />
        <!-- 种族文字 -->
        <p
            class="position-absolute top-550px color-white translate-x-5px z-4 font-size-22px font-family-GBJenLei hearth-stroke-1.2px pointer-events-none"
        >
            {{ store.cardKind }}
        </p>
    </template>
    ...
    >
        <img
            src="@/assets/materials/minion/dual-minion-kind.png"
        ...
        <!-- 种族2文字 -->
        <p
            class="position-absolute top-570px color-white translate-x-5px z-4 font-size-22px font-family-GBJenLei hearth-stroke-1.2px pointer-events-none"
        >
    ...
    <!-- 随从名称框 -->
    <img
        src="@/assets/materials/minion/minion-name.png"
    ...
    <!-- 无水印时 -->
    <img
        v-if="store.watermark === WaterMark.None"
        src="@/assets/materials/minion/minion-description-area.png"
    ...
    <template v-if="store.description">
        <div
            class="position-absolute top-420px w-280px h-141px z-5 flex justify-center items-center"
        >
            <h3
                :class="`position-relative top-0 left-0 m-0 m-auto w-fit h-fit white-space-pre-wrap
    ...
    <!-- 法力值消耗水晶 -->
    <img
        src="@/assets/materials/cost/cost-crystal.png"
        class="position-absolute top-60px translate-x--160px z-2 pointer-events-none"
    />

    <!-- 法力值消耗数值 -->
    <div
        :class="`position-absolute top--30px z-3 font-size-57px pointer-events-none
    ...
    <div
        :class="`position-absolute top-459px z-4 font-size-48px
    ...
    <!-- 卡面图片 -->
    <div v-if="store.cardFaceUrl" class="card-face-clip">
        <CardFace :url="store.cardFaceUrl" :card-type="CardType.Minion" />
    </div>
    <!-- 遮罩 -->
    <Mask
        absolute
...
const minionBaseURL = computed<string>(
    () =>
        new URL(
            `../../assets/minions/single-class/${store.cardClass}-minion.png`,
            import.meta.url,
        ).href,
);
...
const dualClassMinionLeftURL = computed<string>(
    () =>
        new URL(
            `../../assets/minions/dual-class/left/${store.dualCardClass.left}-minion-left.png`,
            import.meta.url,
        ).href,
);
// 右卡底
const dualClassMinionRightURL = computed<string>(
    () =>
        new URL(
            `../../assets/minions/dual-class/right/${store.dualCardClass.right}-minion-right.png`,
            import.meta.url,
        ).href,
);
...
const rarityCrystalURL = computed<string>(() => {
    ...
    return new URL(
        `../../assets/materials/rarity/${store.rarity}-crystal.png`,
        import.meta.url,
    ).href;
});
...
const flagPicture = computed<string>(
    () =>
        new URL(
            `../../assets/materials/flag/${store.flag}-flag.png`,
            import.meta.url,
        ).href,
);
